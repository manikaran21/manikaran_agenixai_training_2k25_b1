...
def data_preprocessing(data):
    ...
    data['RenewableEnergySource'] = data['RenewableEnergySource'].fillna('None')

    data_categorical_encoed = pd.get_dummies(data , columns = categorical_columns).replace({False:0 , True:1})
    data_encoded = data_categorical_encoed.drop(['BuildingID'] , axis = 1)
...
def model_training(data):
    ...
    X = data.drop(columns=['EnergyConsumption'] , axis = 1)
    ...
    print("Feature Importance : Energy Consumption")
...
def model_evaluation(actial_target_data , predicted_target_data):
    ...
    eval_metrics = r2_score(actial_target_data , predicted_target_data)

    print("Model Evaluation Mertrics: \n R2 Score : \n" , eval_metrics)
...
def save_model(model):
    ...
    with open('./Models/energy_consumption_model.pkl',  'wb') as f:
        pickle.dump(model, f)
...
def main():
    file_path = "./Data/building_energy_data.csv"
